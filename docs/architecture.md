...
`run-snapshot-tests.cjs` regenerates each Build Script from the current wizard output, normalizes the generated timestamp and line endings (CRLF → LF), then diffs it against the stored reference. Any difference fails CI immediately and prints a unified diff of the first failure.
