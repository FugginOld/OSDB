...
const SCRIPT_GENERATOR_VERSION = '2026.04.29.1';
...
const BASES = {
  ...
  'arch': {
    ...
    installers: ['calamares','archinstall','archiso-scripted','arch-manual','none'],
  ...
  'arch-arm': {
    ...
    installers: ['archiso-scripted','arch-manual','none'],
  ...
  'fedora-39': {
    ...
    installers: ['anaconda','anaconda-text','kickstart','none'],
  ...
  'rpios-lite-bookworm': {
    ...
    areas: 'main',
    des: ['none'],
    installers: ['raspberry-pi-imager','raspi-config','pi-gen','none'],
    repoTypes: ['official','custom'],
    serviceManager: 'systemd',
    imageType: 'img',
    piGenTier: 'lite',
  ...
  'ubuntu-rpi-2204': {
    ...
    installers: ['raspberry-pi-imager','cloud-init','none'],
    repoTypes: ['official','ppa','custom'],
    serviceManager: 'systemd',
    imageType: 'img',
  ...
  'alarm-rpi4': {
    ...
    des: ['kde','xfce','none'],
    installers: ['alarm-image-scripts','arch-manual','none'],
    repoTypes: ['official','aur'],
    serviceManager: 'systemd',
    imageType: 'img',
  ...
  'gentoo': {
    ...
    installers: ['catalyst','stage3-scripted','gentoo-handbook','none'],
    repoTypes: ['official','custom'],
...
const INSTALLER_DESC = {
  'debian-installer-gtk':   'Debian graphical installer for desktop-style installs.',
  calamares:                'Cross-distro GUI installer for custom live ISOs.',
  'debian-installer-tui':   'Debian text installer for server and headless setups.',
  preseed:                  'Debian automated install via preseed configuration.',
  'ubuntu-desktop-installer':'Ubuntu GUI installer for modern desktop images.',
  ubiquity:                 'Legacy Ubuntu GUI installer used by older releases.',
  subiquity:                'Ubuntu guided text installer for server workflows.',
  'ubuntu-autoinstall':     'Unattended Ubuntu install using autoinstall/cloud-init.',
  archinstall:              'Official Arch guided installer in a text UI.',
  'archiso-scripted':       'Scripted Arch deployment from a custom archiso build.',
  'arch-manual':            'Manual Arch setup for advanced users.',
  anaconda:                 'Fedora graphical installer for standard deployments.',
  'anaconda-text':          'Fedora text-mode Anaconda installer.',
  kickstart:                'Fedora unattended installation with Kickstart files.',
  'raspberry-pi-imager':    'Write prepared OS images to SD/USB media.',
  'raspi-config':           'Post-boot Raspberry Pi system configuration utility.',
  'pi-gen':                 'Build Raspberry Pi OS images from pi-gen.',
  'cloud-init':             'First-boot provisioning with cloud-init.',
  'alarm-image-scripts':    'Scripted Arch Linux ARM image setup workflow.',
  'yast-installer':         'openSUSE graphical YaST installer.',
  'yast-ncurses':           'openSUSE YaST installer in ncurses text mode.',
  autoyast:                 'openSUSE automated installs with AutoYaST profiles.',
  catalyst:                 'Gentoo build pipeline using Catalyst.',
  'stage3-scripted':        'Scripted Gentoo Stage3-based installation flow.',
  'gentoo-handbook':        'Manual Gentoo installation following the handbook.',
  none:                     'No bundled installer; use live/manual/image workflow.',
};

const INSTALLER_LABELS = {
  'debian-installer-gtk': 'Debian Installer GTK',
  calamares: 'Calamares Live Installer',
  'debian-installer-tui': 'Debian Installer TUI',
  preseed: 'Debian Preseed',
  'ubuntu-desktop-installer': 'Ubuntu Desktop Installer',
  ubiquity: 'Ubiquity Legacy Installer',
  subiquity: 'Subiquity',
  'ubuntu-autoinstall': 'Ubuntu Autoinstall / cloud-init',
  archinstall: 'archinstall',
  'archiso-scripted': 'archiso Scripted Install',
  'arch-manual': 'Manual Arch Install',
  anaconda: 'Anaconda Installer',
  'anaconda-text': 'Anaconda Text Mode',
  kickstart: 'Kickstart',
  'raspberry-pi-imager': 'Raspberry Pi Imager',
  'raspi-config': 'raspi-config / first boot configuration',
  'pi-gen': 'pi-gen Image Builder',
  'cloud-init': 'cloud-init',
  'alarm-image-scripts': 'ALARM Image Scripts',
  'yast-installer': 'YaST Installer',
  'yast-ncurses': 'YaST ncurses Installer',
  autoyast: 'AutoYaST',
  catalyst: 'Gentoo Catalyst Builder',
  'stage3-scripted': 'Stage3 Scripted Install',
  'gentoo-handbook': 'Gentoo Handbook Manual Install',
  none: 'No Installer',
};
...
function renderStepInstaller() {
  ...
  let html = `<h2 class="step-heading">Step 5 — Installer</h2>
    <p class="step-sub">Choose an installer workflow (GUI, TUI, scripted, image, or manual), or <em>None</em>.</p>
  ...
  for (const ins of avail) {
    ...
    html += `<div class="tile ${sel}" data-installer="${esc(ins)}" role="radio" tabindex="0" aria-checked="${sel === 'selected'}">
      <div class="tile-title">${esc(INSTALLER_LABELS[ins] || ins)}</div>
...
function scriptHeader(name, builder, containerImage) {
  ...
  const headerMetadata = () => `#!/usr/bin/env bash
# ============================================================
# ${name} — build-distro.sh
# Generated by https://fugginold.github.io/OSDB/
# Generator Version: ${SCRIPT_GENERATOR_VERSION}
# Generated At (UTC): ${generatedAt}
# Builder: ${builder}
# ============================================================\n`;

  const strictBash = () => `set -euo pipefail\n\n`;

  const envVars = () => `DISTRO_NAME="${name}"
...
SCRIPT_START_DIR="\$(pwd -P)"\n\n`;
  ...
  const logAndDieHelpers = () => `log() { printf '\\033[1;34m[%s]\\033[0m %s\\n' "$(date +%T)" "$*"; }
die() { printf '\\033[1;31m[ERROR]\\033[0m %s\\n' "$*" >&2; exit 1; }\n`;

  const cleanupAndTraps = () => `cleanup_build_dir() {
...
start_logging() {
  ...
  const rootCheck = () => `if [ "\${EUID:-\$(id -u)}" -ne 0 ]; then
  die "This script must be run as root. Re-run with: sudo \$0"
fi\n\n`;

  const pathCanonicalization = () => `mkdir -p "\${BUILD_DIR}" "\${OUTPUT_DIR}"
...
${selfHealingBashFragment({
  ...
  buildBlock: `log "Running lorax (this may take 30–60 minutes)..."
rm -rf "$LORAX_OUT"
...
${selfHealingBashFragment({
  ...
  buildBlock: `log "Building openSUSE image (this may take 30–60 minutes)..."
