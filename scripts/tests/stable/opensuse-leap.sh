...
state.ppaList = '';
state.installer = "yast-installer";
...
NODE_EOF
  ...
  if ! grep -Fq "NetworkManager" "$TMP_SCRIPT"; then FAILS+=("$BASE_ID/$DE:missing_default_service_pkg:NetworkManager"); fi
