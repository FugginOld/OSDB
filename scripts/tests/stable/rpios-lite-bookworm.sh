...
NODE_EOF
  ...
  if ! grep -Fq "rpi-eeprom" "$TMP_SCRIPT"; then FAILS+=("$BASE_ID/$DE:missing_default_pkg:rpi-eeprom"); fi
  ...
  if ! grep -Fq "raspi-config" "$TMP_SCRIPT"; then FAILS+=("$BASE_ID/$DE:missing_default_service_pkg:raspi-config"); fi
  if ! grep -Fq "openssh-server" "$TMP_SCRIPT"; then FAILS+=("$BASE_ID/$DE:missing_default_service_pkg:openssh-server"); fi
  ...
  if ! grep -Fq "raspi-config.service" "$TMP_SCRIPT"; then FAILS+=("$BASE_ID/$DE:missing_default_service_unit:raspi-config.service"); fi
  if ! grep -Fq "ssh.service" "$TMP_SCRIPT"; then FAILS+=("$BASE_ID/$DE:missing_default_service_unit:ssh.service"); fi
